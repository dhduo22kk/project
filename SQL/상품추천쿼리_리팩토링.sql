...
================================================================================
  ...
  16-17. [제거] CRTINFO_PRED/NRDINFO_PRED → PRMCNT_PRED에 CTM_AGE/CTM_SEX/NRD_AGE/NRD_SEX 통합
...
FROM (
    ...
    FROM GDREC_ORIGIN_INS_CR_PRED A1
...
-- ※ GD_TYPE은 GDREC_ORIGIN_INS_CR_PRED에서 이미 결정됨 → CASE WHEN 제거
...
/* ============================================================================
   [12] 고객별 계약수/보험료 합계 + 계약자/피보험자 연령/성별
   ※ [최적화] GDREC_ORIGIN_INS_CRTINFO_PRED(16) + GDREC_ORIGIN_INS_NRDINFO_PRED(17) 통합
              CUS_CTM 2회 조인으로 중간 테이블 2개 제거
============================================================================ */
...
SELECT
    ...
    , SUM(A.LTRM_MPY_CV_PRM)  AS PRM
    , COUNT(DISTINCT A.PLYNO) AS PLY_CNT
    , MAX(CASE WHEN F_ISDATE(F_BIRTH_DAY_DEC(B1.CTM_DSCNO)) = 'T'
               THEN TRUNC(MONTHS_BETWEEN(F_LAST_DAY(A.CLS_YYMM),
                          TO_DATE(F_BIRTH_DAY_DEC(B1.CTM_DSCNO),'YYYYMMDD')) / 12)
          END) AS CTM_AGE
    , MAX(CASE WHEN F_SEX_DEC(B1.CTM_DSCNO) = 'M' THEN '1.M'
               WHEN F_SEX_DEC(B1.CTM_DSCNO) = 'F' THEN '2.F' END) AS CTM_SEX
    , MAX(CASE WHEN F_ISDATE(F_BIRTH_DAY_DEC(B2.CTM_DSCNO)) = 'T'
               THEN TRUNC(MONTHS_BETWEEN(F_LAST_DAY(A.CLS_YYMM),
                          TO_DATE(F_BIRTH_DAY_DEC(B2.CTM_DSCNO),'YYYYMMDD')) / 12)
          END) AS NRD_AGE
    , MAX(CASE WHEN F_SEX_DEC(B2.CTM_DSCNO) = 'M' THEN '1.M'
               WHEN F_SEX_DEC(B2.CTM_DSCNO) = 'F' THEN '2.F' END) AS NRD_SEX
FROM GDREC_ORIGIN_INS_CR_PRED A
LEFT JOIN CUS_CTM B1 ON A.CRT_CTMNO      = B1.CTMNO
LEFT JOIN CUS_CTM B2 ON A.MN_NRDPS_CTMNO = B2.CTMNO
GROUP BY A.CLS_YYMM, A.CRT_CTMNO, A.MN_NRDPS_CTMNO;
...
SELECT
    ...
    , MAX(CASE WHEN F_ISDATE(F_BIRTH_DAY_DEC(B.CTM_DSCNO)) = 'T'
...
FROM (SELECT CRT_CTMNO, PLYNO, CLS_YYMM FROM GDREC_ORIGIN_INS_CR_PRED
...
FROM (SELECT CRT_CTMNO, CLS_YYMM FROM GDREC_ORIGIN_INS_CR_PRED
      GROUP BY CRT_CTMNO, CLS_YYMM) A
...
-- ※ [16][17] GDREC_ORIGIN_INS_CRTINFO_PRED / NRDINFO_PRED 제거
--            PRMCNT_PRED에 CTM_AGE/CTM_SEX/NRD_AGE/NRD_SEX 통합됨
...
SELECT DISTINCT
    ...
    , G.GD_TYPE
    , C.INS_ST
FROM (SELECT CRT_CTMNO, MN_NRDPS_CTMNO, CLS_YYMM FROM GDREC_ORIGIN_INS_CR_PRED
      GROUP BY CRT_CTMNO, MN_NRDPS_CTMNO, CLS_YYMM) A
...
-- ※ CTM_AGE/CTM_SEX/NRD_AGE/NRD_SEX는 PRMCNT_PRED(A)에 통합됨 → B7/B77 조인 제거
SELECT
    A.*
