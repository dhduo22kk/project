...
-- ※ DISTINCT→GROUP BY, F_BIRTH_DAY_DEC 인라인 뷰로 1회 계산 후 재사용
CREATE TABLE GDREC_ORIGIN_INS_NRD_PRED AS
SELECT
    X.CLS_YYMM, X.CRT_CTMNO
    , COUNT(DISTINCT X.CTMNO) AS NRD_CNT
    , MAX(CASE WHEN F_ISDATE(X.BRTH) = 'T'
               AND TRUNC(MONTHS_BETWEEN(F_LAST_DAY(X.CLS_YYMM),
                         TO_DATE(X.BRTH,'YYYYMMDD')) / 12) < 20
          THEN 1 ELSE 0 END) AS KIDS_YN
FROM (
    SELECT A.CLS_YYMM, A.CRT_CTMNO, B.CTMNO
        , F_BIRTH_DAY_DEC(B.CTM_DSCNO) AS BRTH
...
GROUP BY X.CLS_YYMM, X.CRT_CTMNO;
...
-- ※ DISTINCT→GROUP BY, F_BIRTH_DAY_DEC/F_SEX_DEC 인라인 뷰로 1회 계산 후 재사용
CREATE TABLE GDREC_ORIGIN_INS_CRTINFO_PRED AS
SELECT A.CRT_CTMNO, A.CLS_YYMM
    , CASE WHEN F_ISDATE(B.BRTH) = 'T'
           THEN TRUNC(MONTHS_BETWEEN(F_LAST_DAY(A.CLS_YYMM), TO_DATE(B.BRTH,'YYYYMMDD')) / 12)
      END AS CTM_AGE
    , CASE WHEN B.SEX = 'M' THEN '1.M' WHEN B.SEX = 'F' THEN '2.F' END AS CTM_SEX
FROM (SELECT CRT_CTMNO, CLS_YYMM FROM GDREC_ORIGIN_INS_CR_PRED
      GROUP BY CRT_CTMNO, CLS_YYMM) A
LEFT JOIN (SELECT CTMNO,
                  F_BIRTH_DAY_DEC(CTM_DSCNO) AS BRTH,
                  F_SEX_DEC(CTM_DSCNO)       AS SEX
           FROM CUS_CTM) B ON B.CTMNO = A.CRT_CTMNO;

DROP TABLE GDREC_ORIGIN_INS_NRDINFO_PRED PURGE;
CREATE TABLE GDREC_ORIGIN_INS_NRDINFO_PRED AS
SELECT A.MN_NRDPS_CTMNO, A.CLS_YYMM
    , CASE WHEN F_ISDATE(B.BRTH) = 'T'
           THEN TRUNC(MONTHS_BETWEEN(F_LAST_DAY(A.CLS_YYMM), TO_DATE(B.BRTH,'YYYYMMDD')) / 12)
      END AS CTM_AGE
    , CASE WHEN B.SEX = 'M' THEN '1.M' WHEN B.SEX = 'F' THEN '2.F' END AS CTM_SEX
FROM (SELECT MN_NRDPS_CTMNO, CLS_YYMM FROM GDREC_ORIGIN_INS_CR_PRED
      GROUP BY MN_NRDPS_CTMNO, CLS_YYMM) A
LEFT JOIN (SELECT CTMNO,
                  F_BIRTH_DAY_DEC(CTM_DSCNO) AS BRTH,
                  F_SEX_DEC(CTM_DSCNO)       AS SEX
           FROM CUS_CTM) B ON B.CTMNO = A.MN_NRDPS_CTMNO;
